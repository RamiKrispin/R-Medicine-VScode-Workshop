...
Where the `-v` stands for volume followed by the local path and the destination path on the image, seperated by `:`. For example, let's mount from the repo the `scripts` folder to the container and run the `hello-world.R` file:
...
Likewise, you can mount many volumes follow the same method. For example, let's mount, in addition to the `scripts` folder the `data` folder:

``` shell
docker run --interactive --tty \                                                                                                                                                                         ok  33s
